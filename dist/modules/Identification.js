...
import { i18nFormat } from "../init.js";
...
export default class Identification {
    ...
    static async mystifyAdvancedDialog(itemUuid, source = undefined) {
        ...
        const dialog = new Dialog({
            ...
            close: (html) => {
                ...
                let formData = Object.fromEntries(Object.entries(formDataBase.toObject()).filter(e => e[1] !== false));
    ...
    static _getTypeProperties(origData) {
        let defaultProperties = getGame().settings.get(FORIEN_UNIDENTIFIED_ITEMS_MODULE_NAME, "itemProperties");
    ...
    static _getMystifiedMeta(origData) {
        let iconSettings = getGame().settings.get(FORIEN_UNIDENTIFIED_ITEMS_MODULE_NAME, "defaultIcons");
        let iconType = getProperty(iconSettings, origData.type) || `/modules/${FORIEN_UNIDENTIFIED_ITEMS_MODULE_NAME}/icons/${FORIEN_UNIDENTIFIED_ITEMS_DEFAULT_ICON}`;
        return {
